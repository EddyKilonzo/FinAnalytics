...
export class TransactionsController {
  ...
  async findAll(@Query() query: TransactionQueryDto, @Request() req: AuthRequest) {
    try {
      const { transactions, total, page, limit, totalPages } =
        await this.transactionsService.findAll(query, req.user.id, req.user.role);
      ...
      throw new InternalServerErrorException('Could not retrieve transactions. Please try again.');
    }
  }
  ...
  @ApiOperation({
    ...
    description: 'Returns total income, total expenses, and net balance. Optionally filtered by date range.',
  })
  @ApiQuery({ name: 'dateFrom', required: false, type: String, example: '2026-01-01' })
  ...
  async getSummary(
    ...
    try {
      const data = await this.transactionsService.getSummary(req.user.id, dateFrom, dateTo);
      return { success: true, data };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error('Unexpected error in getSummary', error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not retrieve summary. Please try again.');
    }
  }
  ...
  @ApiOperation({
    ...
    description: 'Returns expense totals grouped by category — used for pie and bar charts. Sorted highest to lowest.',
  })
  ...
  async getByCategory(
    ...
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error('Unexpected error in getByCategory', error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not retrieve category breakdown. Please try again.');
    }
  }
  ...
  @ApiOperation({
    ...
    description: 'Response includes the ML-suggested category (if any) alongside the user-confirmed category.',
  })
  @ApiParam({ name: 'id', description: 'Transaction CUID' })
  @ApiResponse({ status: 200, description: 'Transaction found.' })
  @ApiResponse({ status: 401, description: 'Missing or invalid token.', type: ErrorResponseDto })
  @ApiResponse({ status: 403, description: 'Not your transaction or onboarding incomplete.', type: ErrorResponseDto })
  @ApiResponse({ status: 404, description: 'Not found.', type: ErrorResponseDto })
  async findOne(@Param('id') id: string, @Request() req: AuthRequest) {
    try {
      const data = await this.transactionsService.findById(id, req.user.id, req.user.role);
      return { success: true, data };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(`Unexpected error fetching transaction [${id}]`, error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not retrieve transaction. Please try again.');
    }
  }
  ...
  async create(@Body() dto: CreateTransactionDto, @Request() req: AuthRequest) {
    try {
      ...
      return { success: true, message: 'Transaction recorded successfully', data };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error('Unexpected error creating transaction', error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not record transaction. Please try again.');
    }
  }
  ...
  @ApiResponse({ status: 400, description: 'Validation error.', type: ErrorResponseDto })
  @ApiResponse({ status: 401, description: 'Missing or invalid token.', type: ErrorResponseDto })
  @ApiResponse({ status: 403, description: 'Not your transaction.', type: ErrorResponseDto })
  @ApiResponse({ status: 404, description: 'Not found.', type: ErrorResponseDto })
  async update(
    @Param('id') id: string,
    ...
    try {
      const data = await this.transactionsService.update(id, dto, req.user.id, req.user.role);
      return { success: true, message: 'Transaction updated successfully', data };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(`Unexpected error updating transaction [${id}]`, error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not update transaction. Please try again.');
    }
  }
  ...
  @ApiResponse({ status: 400, description: 'Invalid categoryId.', type: ErrorResponseDto })
  @ApiResponse({ status: 401, description: 'Missing or invalid token.', type: ErrorResponseDto })
  @ApiResponse({ status: 403, description: 'Not your transaction.', type: ErrorResponseDto })
  @ApiResponse({ status: 404, description: 'Transaction or category not found.', type: ErrorResponseDto })
  async correctCategory(
    ...
    try {
      ...
      return {
        success: true,
        message: 'Category corrected. Thank you — this helps improve future suggestions.',
        data,
      };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(`Unexpected error correcting category on tx [${id}]`, error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not correct category. Please try again.');
    }
  }
  ...
  async remove(@Param('id') id: string, @Request() req: AuthRequest) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(`Unexpected error deleting transaction [${id}]`, error instanceof Error ? error.stack : String(error));
      throw new InternalServerErrorException('Could not delete transaction. Please try again.');
    }
  }
}
