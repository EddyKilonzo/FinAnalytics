...
export class LessonsController {
  ...
  async findAll() {
    try {
      ...
      this.logger.error('Unexpected error in lessons findAll', error instanceof Error ? error.stack : String(error));
  ...
  @ApiResponse({ status: 200, description: 'List of suggested lessons with reason.' })
  @ApiResponse({ status: 401, description: 'Missing or invalid token.', type: ErrorResponseDto })
  @ApiResponse({ status: 403, description: 'Onboarding not completed.', type: ErrorResponseDto })
  async getSuggested(@Query('context') context: string, @Request() req: AuthRequest) {
    try {
      const data = await this.lessonsService.getSuggested(context ?? '', req.user.id);
      return { success: true, data };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(
      ...
      throw new InternalServerErrorException('Could not retrieve suggested lessons.');
    }
  }
  ...
  async findOne(@Param('id') id: string) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(`Unexpected error fetching lesson [${id}]`, error instanceof Error ? error.stack : String(error));
