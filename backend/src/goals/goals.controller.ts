...
export class GoalsController {
  ...
  async getDashboard(@Request() req: AuthRequest) {
    try {
      const data = await this.goalsService.getDashboard(
        req.user.id,
        req.user.role,
      );
      ...
      this.logger.error(
        "Unexpected error in getDashboard",
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
        "Could not retrieve goals dashboard. Please try again.",
      );
    }
  }
  ...
  async findAll(@Request() req: AuthRequest) {
    try {
      ...
      throw new InternalServerErrorException(
        "Could not retrieve goals. Please try again.",
      );
    }
  }
  ...
  @ApiResponse({
    status: 403,
    description: "Not your goal or onboarding incomplete.",
    type: ErrorResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  async findOne(@Param("id") id: string, @Request() req: AuthRequest) {
    try {
      const data = await this.goalsService.findById(
        id,
        req.user.id,
        req.user.role,
      );
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error fetching goal [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
        "Could not retrieve goal. Please try again.",
      );
    }
  }
  ...
  async create(@Body() dto: CreateGoalDto, @Request() req: AuthRequest) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        "Unexpected error creating goal",
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
  ...
  @ApiResponse({
    status: 200,
    description: "Savings allocated. Returns updated goal.",
  })
  @ApiResponse({
    status: 400,
    description: "Validation error.",
  ...
  async allocate(
    ...
    try {
      const data = await this.goalsService.allocate(
        id,
        dto,
        req.user.id,
        req.user.role,
      ...
      return {
        success: true,
        message: `KES ${dto.amount} allocated successfully`,
        data,
      };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error allocating to goal [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
        "Could not allocate savings. Please try again.",
      );
    }
  }
  ...
  @ApiResponse({
    status: 200,
    description: "Withdrawal applied. Returns updated goal.",
  })
  @ApiResponse({
    status: 400,
  ...
  async withdraw(
    ...
    try {
      const data = await this.goalsService.withdraw(
        id,
        dto,
        req.user.id,
        req.user.role,
      ...
      return {
        success: true,
        message: `KES ${dto.amount} withdrawn from goal`,
        data,
      };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error withdrawing from goal [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
  ...
  async update(
    ...
    try {
      const data = await this.goalsService.update(
        id,
        dto,
        req.user.id,
        req.user.role,
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error updating goal [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
        "Could not update goal. Please try again.",
      );
    }
  }
  ...
  async remove(@Param("id") id: string, @Request() req: AuthRequest) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error deleting goal [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
