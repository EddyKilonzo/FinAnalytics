...
export class BudgetsController {
  ...
  @ApiResponse({
    status: 200,
    description:
      "Object with budgetAlerts (array) and nudges (array of { id, type, message, severity }).",
  })
  @ApiResponse({
    status: 401,
  ...
  async getAlerts(@Request() req: AuthRequest) {
    try {
      ...
      const data = await this.budgetsService.getAlerts(
        req.user.id,
        req.user.role,
        goalNudges,
      );
      ...
      this.logger.error(
        "Unexpected error in getAlerts",
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
        "Could not retrieve budget alerts. Please try again.",
      );
    }
  }
  ...
  @ApiResponse({
    status: 200,
    description: "Budget list with live spending summaries.",
  })
  @ApiResponse({
    status: 401,
    description: "Missing or invalid token.",
  ...
  async findAll(@Request() req: AuthRequest) {
    try {
      ...
      throw new InternalServerErrorException(
        "Could not retrieve budgets. Please try again.",
      );
    }
  }
  ...
  @ApiResponse({
    status: 403,
    description: "Not your budget or onboarding incomplete.",
    type: ErrorResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  async findOne(@Param("id") id: string, @Request() req: AuthRequest) {
    try {
      const data = await this.budgetsService.findById(
        id,
        req.user.id,
        req.user.role,
      );
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error fetching budget [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
        "Could not retrieve budget. Please try again.",
      );
    }
  }
  ...
  async update(
    ...
    try {
      const data = await this.budgetsService.update(
        id,
        dto,
        req.user.id,
        req.user.role,
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error updating budget [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
  ...
  async remove(@Param("id") id: string, @Request() req: AuthRequest) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Unexpected error deleting budget [${id}]`,
        error instanceof Error ? error.stack : String(error),
      );
      throw new InternalServerErrorException(
