...
async function bootstrap() {
  ...
  app.use((req: { path: string }, _res: unknown, next: () => void) => {
    ...
    return (helmet() as (req: unknown, res: unknown, next: () => void) => void)(
      req,
      _res,
      next,
    );
    ...
    .addBearerAuth(
      {
        ...
        bearerFormat: "JWT",
        description:
          "Paste the JWT access token obtained from /auth/login or /auth/signup",
      },
  ...
  logger.log(
    `Health    → http://localhost:${port}/api/v1/health (backend + ML status)`,
  );
